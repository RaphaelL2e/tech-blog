---
title: 单链表
date: 2019-09-30 22:15:44+08:00
updated: '2019-09-30T22:15:44+08:00'
description: 链表是以节点的方式来存储，链式存储 每个节点包含data域，next域：指向下一个节点 如图：发现链表的各个节点不一定是连续存储 链表分带头节点的链表和没有头节点的链表，根据实际的需求来 先创建一个head头节点，作用就是表示单链表的头
  后面每添加一个节点，就直接加入到链表的最后遍历 package。

...
skill_groups = [
    ...
    ('大数据',   ['ClickHouse', 'Elasticsearch', 'Kafka', 'NebulaGraphDB', 'MongoDB']),
    ('关系型/缓存', ['MySQL', 'Redis']),
...
jobs = [
    ('北京·京东远升科技', '软件开发工程师', '2025.04 — 至今', LGRAY,
     ...
     [
         ...
         ('趋势图看板性能重构（SQL↓85%）',
          '将 13 条独立 SQL 按维度归类合并为 2 条通用聚合 SQL，后端逻辑驱动配置化组合；'
          '前端首屏加载降低 75%，接口 P99 从 3000ms 降至 600ms；'
          '改造后系统可维护性大幅提升，后续新增指标零代码改动'),

         ('灭霸看板维度扩展',
          '设计维度扩展配置表 + 动态字段映射机制，支持新渠道维度热插拔上线；'
          '改造 10 个核心指标关联逻辑，新维度上线周期从 2 周压缩至 1 天'),
     ]),
